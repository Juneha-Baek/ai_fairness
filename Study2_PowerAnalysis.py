...
standard errors (CRV1) at the team level. Fixed effects are absorbed via the
within-cluster demeaning algorithm implemented in `pyfixest`:

  - H6, H8 (cross-phase): no fixed effects, because `is_group` is a
    team-level variable and would be absorbed by team fixed effects. Cluster-
    robust SE at the team level still accounts for within-team correlation.
  - H7, H9, H10, H11 (group phase only): team fixed effects, which absorb
    between-team variation in baseline selection tendencies. The
    treatment-by-gender interactions of interest have within-team variation
    (through delta_gender) and are therefore identified.
...
# Estimation approach:
#   - H6, H8 (cross-phase): no fixed effects, because `is_group` is a
#     team-level variable that would be absorbed by team fixed effects.
#     Cluster-robust SE at the team level still accounts for within-team
#     correlation.
#   - H7, H9, H10, H11 (group phase only): team fixed effects, which absorb
#     between-team variation in baseline selection tendencies. The
#     interactions of interest have within-team variation through delta_gender
#     and are therefore identified.
# Pre-computed interaction columns avoid relying on pyfixest's `:` operator.
...
def test_hypothesis_h6(df, alpha=0.05):
    ...
    A negative coefficient on delta_gender x is_group supports H6.
    """
    try:
        df_test = df.copy()
        ...
        formula = (
            ...
            "delta_university + delta_gpa + is_group_f + dg_isgrp"
        )
...
def test_hypothesis_h8(df, alpha=0.05):
    ...
    delta_gender x is_group x has_ai supports H8.
    """
    try:
        df_test = df.copy()
        df_test['is_group_f'] = df_test['is_group'].astype(float)
        ...
        df_test['isgrp_ai'] = df_test['is_group_f'] * df_test['has_ai_f']
        df_test['dg_isgrp_ai'] = df_test['delta_gender'] * df_test['is_group_f'] * df_test['has_ai_f']

        formula = (
            "y ~ delta_gender + delta_internship_exp + delta_certificates + "
            "delta_university + delta_gpa + is_group_f + has_ai_f + isgrp_ai + "
            "dg_isgrp + dg_ai + dg_isgrp_ai"
        )
        model = pf.feglm(fml=formula, data=df_test, family="logit",
                          vcov={"CRV1": "team_id"})
