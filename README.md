...
Making Structure."** The repository contains two scripts, one per study,
together with a Windows helper that runs them in parallel.
...
where the hypothesis is directional. For H1 and H2, Holm-Bonferroni
correction is applied within the family of contrasts.

All hypothesis tests are estimated by logistic regression with cluster-robust
standard errors (CRV1). Fixed effects are absorbed via the within-cluster
demeaning algorithm implemented in `pyfixest`.
...
| Study 2 H6, H8 (cross-phase) | None | Team |
| Study 2 H7, H9, H10, H11 (group phase only) | Team | Team |

For Study 2 H6 and H8, the focal interaction includes `is_group`, which is a
team-level variable that would be absorbed by team fixed effects. These models
therefore use cluster-robust SE at the team level without absorbing fixed
effects. For H7, H9, H10, and H11, the focal interactions include
`delta_gender`, which has within-team variation and is identified after
absorbing team fixed effects.
...
Each script accepts a `--mode` flag that selects which hypotheses to test and
which sample-size range to scan. Modes exist because H4, H5, H10, and H11
require larger samples than the other hypotheses; running everything in one
pass would be inefficient.
...
Python scripts and double-click it. Total wall-clock time is roughly the
...
Each run writes its outputs to a timestamped folder named
